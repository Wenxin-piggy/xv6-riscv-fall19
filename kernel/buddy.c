...
//mychange:xor for alloc
void bit_myallocset(char *array, int index){
  char b = array[index/8];
  char m = (1 << (index % 8));
   array[index/8] = (b ^ m);
}
//mychange:isset for allco
int bit_myisset(char *array, int index){
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
}
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_myallocset(bd_sizes[k].alloc, blk_index(k, p)/2);
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_myallocset(bd_sizes[k-1].alloc, blk_index(k-1, p)/2);
    lst_push(&bd_sizes[k-1].free, q);
  }
  release(&lock);
  // printf("get out bd_malloc\n");
  return p;
}
...
bd_free(void *p) {
  void *q;
  int k;
  // printf("get in bd_free\n");
  acquire(&lock);
  for (k = size(p); k < MAXSIZE; k++) {
    // printf("%d er fe zhi yi1\n",k);
    int bi = blk_index(k, p);
    int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
    bit_myallocset(bd_sizes[k].alloc, bi/2);  // free p at size k
    if (bit_myisset(bd_sizes[k].alloc, buddy/2)) {  // is buddy allocated?
    ...
    // printf("%d er fe zhi yi2,%d\n",k,MAXSIZE);
    if(buddy % 2 == 0) {
      p = q;
    }
    // printf("%d er fe zhi yi3,%d\n",k,MAXSIZE);
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      bit_myallocset(bd_sizes[k].alloc, bi/2);
    }
  }
}
...
int
bd_initfree_pair(int k, int bi,int flag) {//0表示处理左边，1表示处理右边
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  //change
  if(bit_myisset(bd_sizes[k].alloc, bi/2)){
    free = BLK_SIZE(k);
    if(flag == 0){//放左边
      //将右边的放进去
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    free += bd_initfree_pair(k, left,0);//0表示处理左边
    if(right <= left)
      continue;
    free += bd_initfree_pair(k, right,1);//1表示处理右边
  }
  return free;
}
